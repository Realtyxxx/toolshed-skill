...
一个 agent skill：把本机上**全局、可复用**的工具（PATH 上的脚本、守护进程、shell hook、全局配置、注册的 skill/hook/MCP/cron）记进 `~/mem.md` + `~/mem/<tool>.md`，跨 agent（claude / codex / gemini）共享。
...
遍历 `~/mem/*.md`，实际执行每篇的 `## 自查` bash 块，并检查 `~/mem.md` 索引里有没有对应链接。
